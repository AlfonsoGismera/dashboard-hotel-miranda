...
const ModalContent = styled.div`
  ...
  padding:1rem; border-radius:0.5rem; position:relative; width:300px;
`;
const CloseButton = styled.button`
  position:absolute; top:-10px; right:-10px;
  width:24px; height:24px; border-radius:50%; background:red; color:#fff;
...
export default function Guests(){
  ...
  const pageSize=10;
  const statusColors={
    'Checked In':'green','Checked Out':'red','Pending':'#b2f31d','Booked':'#0275d8','Cancelled':'#d9534f'
  };
        ...
        <Table>
          ...
          <tbody>
            {paged.map((r,i)=>(<tr key={i}>
              ...
              <Td>{format(new Date(r.checkOut),'MMM do yyyy',{locale})}<br/><small>{format(new Date(r.checkOut),'hh:mm a',{locale})}</small></Td>
              ...
              <Td>{r.roomType}</Td>
              <Td><Button bg={statusColors[r.status]||'orange'} color="#fff" hoverbg="#222">{r.status}</Button></Td>
