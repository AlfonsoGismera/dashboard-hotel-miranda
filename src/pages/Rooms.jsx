...
const NameCell = styled(Td)`display:flex; align-items:center; gap:1rem;
...
const Textarea = styled.textarea`width:100%; padding:0.5rem; border:1px solid #ccc; border-radius:4px;`;
...
export default function Rooms() {
  ...
  const [menu, setMenu] = useState({ visible: false, x: 0, y: 0, room: null });
  ...
  function onMore(e, room) {
    e.preventDefault(); e.stopPropagation();
    const rect = e.currentTarget.getBoundingClientRect();
    setMenu({ visible: true, x: rect.left, y: rect.bottom, room });
  }
  function closeMenu() { setMenu({ ...menu, visible: false }); }

  function handleDelete() {
    dispatch(deleteRoom(menu.room.roomId));
    closeMenu();
  }
  function handleEdit() {
  ...
  return (
    <Page onClick={closeMenu}>
        ...
        <Table>
          ...
          <tbody>
            {paged.map((r,i) => (<tr key={i}>
              <NameCell><img src={r.image} alt={r.roomName}/><div><small>{r.roomId}</small><br/><strong>{r.roomName}</strong></div></NameCell>
              <Td>{r.bedType}</Td>
              <Td>{r.roomFloor}</Td>
      ...
      {menu.visible && <Menu x={menu.x} y={menu.y}><MenuItem onClick={handleEdit}>{t.edit||'Edit'}</MenuItem><MenuItem onClick={handleDelete}>{t.delete||'Delete'}</MenuItem></Menu>}

      {editRoom && (
        <Overlay>
          <Modal onClick={e=>e.stopPropagation()}>
            <CloseIcon size={20} onClick={()=>setEditRoom(null)} />
            <h3>{t.edit||'Edit'} {editRoom.roomName}</h3>
            <Field><Label>Room ID</Label><Input value={editRoom.roomId} disabled/></Field>
            <Field><Label>Room Name</Label><Input value={editRoom.roomName} onChange={e=>setEditRoom({...editRoom,roomName:e.target.value})}/></Field>
            <Field><Label>Bed Type</Label><Input value={editRoom.bedType} onChange={e=>setEditRoom({...editRoom,bedType:e.target.value})}/></Field>
            <Field><Label>Floor</Label><Input value={editRoom.roomFloor} onChange={e=>setEditRoom({...editRoom,roomFloor:e.target.value})}/></Field>
            <Field><Label>Facilities (comma)</Label><Input value={editRoom.facilities.join(',')} onChange={e=>setEditRoom({...editRoom,facilities:e.target.value.split(',')})}/></Field>
            <Field><Label>Rate</Label><Input value={editRoom.rate} onChange={e=>setEditRoom({...editRoom,rate:e.target.value})}/></Field>
            <Field><Label>Status</Label><Input value={editRoom.status} onChange={e=>setEditRoom({...editRoom,status:e.target.value})}/></Field>
            <Field><Label>Image URL</Label><Input value={editRoom.image} onChange={e=>setEditRoom({...editRoom,image:e.target.value})}/></Field>
            <Actions>
              <Button $color="#fff" $bg={theme.subtitle} onClick={()=>setEditRoom(null)}>{t.cancel||'Cancel'}</Button>
              <Button $bg={theme.primary} $color="#fff" onClick={()=>{dispatch(updateRoom(editRoom));setEditRoom(null);}}>{t.save||'Save'}</Button>
            </Actions>
          </Modal>
        </Overlay>
      )}
